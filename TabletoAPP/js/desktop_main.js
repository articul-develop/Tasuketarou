      ...
      try {
        ...
        if (idsToDelete.length > 0) {
          ...
          if (!deleteRequest.ok) {
            ...
            AuthModule.sendErrorLog("削除リクエスト", errorMessage);
          }
        }
      } catch (error) {
        ...
        AuthModule.sendErrorLog("削除処理中", errorMessage);
      }
    }
...
tableRecords.forEach((row, index) => {
  console.log(`[DEBUG] テーブル行 ${index + 1}:`, JSON.stringify(row, null, 2));
      ...
      try {
        ...
        AuthModule.sendErrorLog("当アプリの更新キー項目更新", errorMessage);
        return event;
      }
      ...
      if (deletedIdentifiers.length > 0) {
        try {
          ...
          const errorMessage = error?.message;
          alert(`プラグインエラー：削除処理中にエラーが発生しました。\n${errorMessage}`);
          AuthModule.sendErrorLog("削除処理中(削除された行)", errorMessage);
        }
      }
      ...
      try {
        ...
        if (!response.ok) {
          ...
          AuthModule.sendErrorLog("更新先アプリのレコード取得", errorMessage);
        ...
        AuthModule.sendErrorLog("更新処理", errorMessage);
        return event;
      }
      ...
      if (recordsToUpdate.length > 0) {
        ...
        try {
          ...
          if (!updateResponse.ok) {
            ...
            AuthModule.sendErrorLog("更新先アプリの更新", errorMessage);
          }

        } catch (error) {
          const errorMessage = error?.message;
          alert(`プラグインエラー：更新先アプリへの通信に失敗しました。\n${errorMessage}`);
          AuthModule.sendErrorLog("更新先アプリの通信", errorMessage);
        }
      }
      ...
      if (recordsToCreateInTarget.length > 0) {
        try {
          ...
          if (!createResponse.ok) {
            ...
            AuthModule.sendErrorLog("更新先アプリの新規登録", errorMessage);
          }
        } catch (error) {
          const errorMessage = error?.message;
          alert(`プラグインエラー：更新先アプリへの通信に失敗しました。\n${errorMessage}`);
